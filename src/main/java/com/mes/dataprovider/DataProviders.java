 ...
 * 
 */
package com.mes.dataprovider;

import java.util.HashMap;
import java.util.Map;

import com.mes.utility.NewExcelLibrary;
...
//Class --> LoginPageTest,HomePageTest Test Case--> loginTest, wishListTest, orderHistoryandDetailsTest
	...
	public Object[][] getInvalidCredentials() {
		// Totals rows count
		int rows = obj.getRowCount("invalid_credentials");
		// Total Columns
		int column = obj.getColumnCount("invalid_credentials");
		int actRows = rows - 1;
		...
		for (int i = 0; i < actRows; i++) {
			for (int j = 0; j < column; j++) {
				data[i][j] = obj.getCellData("invalid_credentials", j, i + 2);
			}
		}
		return data;
	}
